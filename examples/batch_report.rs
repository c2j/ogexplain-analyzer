use ogexplain_core::{analyze, model::PlanNode, parse_multi};
...
fn main() {
    ...
    println!(
        "成功解析:         {} ({:.1}%)",
        ok_files,
        ok_files as f64 / total_files as f64 * 100.0
    );
    println!("解析 EXPLAIN 块:  {}", total_blocks);
    println!(
        "无 EXPLAIN 块:    {} ({:.1}%)",
        no_blocks,
        no_blocks as f64 / total_files as f64 * 100.0
    );
    println!(
        "解析失败:         {} ({:.1}%)",
        parse_err,
        parse_err as f64 / total_files as f64 * 100.0
    );
    println!("解析总节点数:     {}", total_nodes);

    println!("\n--- 2. 无 EXPLAIN 块的文件 ---");
    for f in &no_block_files {
        println!("  {}", f);
    }
    ...
    for (sev, count) in &sorted_sev {
        println!(
            ...
            sev,
            count,
            **count as f64 / total_findings.max(1) as f64 * 100.0
        );
    }
